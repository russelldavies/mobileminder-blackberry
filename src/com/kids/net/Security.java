...
/**
 * This class holds the encryption logic and can be called by other classes when encryption or decryption is needed.
 * 
 *
 */
 ...
 */
	public String cryptFull(String inputText, boolean encrypt)
	{
		...
		logger.log("cryptFull<-:"+inputText);
		...
		for(int countOutter = 0; countOutter < charArray.length; countOutter++)
		{
			...
			{	//logger.log(charArray[countOutter]+" AT "+charAt(charArray[countOutter])); 
			charArray[countOutter] = crypt(charArray[countOutter],encrypt);		}
		}	
		logger.log("cryptFull->:["+new String(charArray)+"]");
...
/**
 * This method specifies which character set to be used when encrypting and decrypting.
 * It subtracts 1 from the char value
 * @param index character position in the character set
 * @param encrypt specifies whether the character array should be encrypted or decrpted.
...
/**
 * This method checks if the character that can be used in a URL.
 * 
 * @param inputTarget a character to be checked.
 * @return if the character can't be used in a URL, it will return -1.
 */
	/*
	public int charAt(char inputTarget)
	{
		for(int count = 0; count<charSET.length; count++)
		{
			if(charSET[count] == inputTarget)
			{	return count;	}
		}
		return -1;
	}*/
