...
public class TransportDetective {
    ...
    public static final int DEFAULT_TCP_CELLULAR				= 64;    	// Represents the availability of TCP_CELLULAR service book that has the TCP APN information pre-populated. Please note that this is not a new transport. This simply indicates that your application do  not need to know the APN information for TCP_CELLULAR since the device already knows it via this Service Record. Please see URLFactory.getHttpDefaultTcpCellularUrl(ServiceRecord tcpServiceRecord)   
    ...
    public boolean isDefaultTcpCellularServiceRecordAvailable(){
    	updateTransportServiceAvailability();
    	return ((_availableTransportServices & DEFAULT_TCP_CELLULAR) > 0);
    }
                ...
                if (( transportType != -1 ) ) {
                    if (   (( transportType == TRANSPORT_MDS || ( transportType & (TRANSPORT_BIS_B)) > 0 )
                        ...
                        || (( transportType == TRANSPORT_TCP_WIFI )
                                && ( wifiSupported ))   
                        || (( transportType == DEFAULT_TCP_CELLULAR)
                        		&& ( cellularDataServiceSupported ))        ) {        
	...
	private void updateTransportCoverageAvailability(){ 
		...
		if(CoverageInfo.isCoverageSufficient(CoverageInfo.COVERAGE_DIRECT, RadioInfo.WAF_3GPP|RadioInfo.WAF_CDMA|RadioInfo.WAF_IDEN, false)){
